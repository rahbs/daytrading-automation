...
class CpStockChart:
    ...
    def RequestDWM(self, code, dwm, count, from_date=0, ohlcv_only=True):
        """
        :param code: 종목코드
    ...
    def RequestMT(self, code, dwm, tick_range, count, from_date=0, ohlcv_only=True):
        """
        :param code: 종목 코드
        :param dwm: 'm':분봉, 'T':틱봉
        ...
        while count > rcv_count:
            ...
            print('{} / {}(maximum)'.format(rcv_count, count))
            # 서버가 가진 모든 데이터를 요청한 경우 break.
            # self.objStockChart.Continue 는 개수로 요청한 경우
            # count만큼 이미 다 받았더라도 계속 1의 값을 가지고 있어서
            # while 조건문에서 count > rcv_count를 체크해줘야 함.
